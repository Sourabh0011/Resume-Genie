...
import { FileText, Sparkles, ShieldCheck, ArrowRight, Linkedin, Mail, Loader2, CheckCircle2, LayoutDashboard, Clock, Send } from 'lucide-react';
...
const ResumeGenie = () => {
  ...
  const fetchRequests = async () => {
    try {
      const response = await fetch('https://resume-backend-umber.vercel.app'); // You'll need to add this simple GET route to backend
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch('http://localhost:5000/api/request-resume', {
        method: 'POST',
              ...
              {requests.length > 0 ? requests.map((req, i) => (
                <div key={i} className="bg-slate-900/50 border border-slate-800 p-6 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 group hover:border-slate-700 transition-all">
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <div className={`px-4 py-1.5 rounded-full text-xs font-bold flex items-center gap-2 ${req.status === 'Sent' ? "bg-emerald-500/10 text-emerald-400" : "bg-amber-500/10 text-amber-400"}`}>
...
<footer className="relative z-10 mt-24 border-t border-slate-800/50 bg-slate-900/60 backdrop-blur-2xl">
  <div className="max-w-7xl mx-auto px-8 py-16">
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 items-center">
      ...
      {/* 2. Middle Navigation / Code Links */}
